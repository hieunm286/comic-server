...
const { parsePaginationOption } = require('./search');

const { ObjectId } = require('mongoose').Types;

const getFieldV3 = (field, fieldName) => {
...
const getSearchOption = (args, searchModel) => Object.keys(args).filter((key) => {
    const obj = getFieldV3(searchModel, key)[0];
    return (obj && _.isString(obj));
});

const convertToSearchQuery = (key, value, type) => {
...
const mergeSearchObjToPopulate = (validSearchOption, poppulateObj, searchModel, requestArgs) => {
...
const searchFunction = (args, defaultSortField, searchModel, poppulateObj) => {
